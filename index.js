...
btn.addEventListener("click", function change(e){
    ...
    for(let i =0;i<5;i++){
    fetch(`https://www.thecolorapi.com/scheme/?hex=${convert(colourInput.value)}&count=5&mode=${mode.value}`)
    .then(res => res.json())
...
for(let i =0;i<5;i++){
    ...
    fetch(`https://www.thecolorapi.com/scheme/?hex=${(colourInp)}&count=5&mode=${mode.value}`)
    .then(res => res.json())
    .then(data => {console.log(data.colors[i].hex.value)
        navigator.clipboard
