...
btn.addEventListener("click", function change(e){
    colourInput.style.borderColor=convert(colourInput.value);
    const colorValue = convert(colourInput.value);
    for(let i =0;i<5;i++)
    fetch(`https://www.thecolorapi.com/scheme/?hex=${colorValue.slice(1)}&count=5&mode=${mode.value}`)
    .then(res => res.json())
...
for(let i =0;i<5;i++){
    document.getElementById(`hex-code${i+1}`).addEventListener("click",function copyText(e){
        fetch(`https://www.thecolorapi.com/scheme/?hex=${colorValue}&count=5&mode=${mode.value}`)
        .then(res => res.json())
        .then(data => {console.log(data.colors[i].hex.value);
            navigator.clipboard
